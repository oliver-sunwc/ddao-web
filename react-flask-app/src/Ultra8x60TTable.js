...
function App() {
  ...
  async function handlePriceChange(e) {
      ...
      await axios("/api/data/gpu/3060Ti/"+p)
  ...
  async function resetPrice(e) {
      await axios("/api/data/gpu/3060Ti/"+gpuPrice)
      const result = await axios("/api/data/rig")
      setData(result.data)
      setPrice(gpuPrice)
  }
  ...
  async function handleRigPriceChange(e) {
    ...
    await axios("/api/data/rig/3060Ti/"+p)
    const result = await axios("/api/data/rig")
    console.log(result.data)
    setData(result.data)
    setServerP(p)
...
async function resetRigPrice(e) {
    await axios("/api/data/rig/3060Ti/"+serverCost)
    const result = await axios("/api/data/rig")
    setData(result.data)
    ...
    () => [
      {
        ...
        columns: [
          ...
          {
            Header: "Ethereum",
            accessor: "gpu.3060Ti.Ethereum"
          },
          {
            Header: "Conflux",
            accessor: "gpu.3060Ti.Conflux"
          },
          {
            Header: "Ravencoin",
            accessor: "gpu.3060Ti.Ravencoin"
          },
          {
            Header: "Bitcoin Gold",
            accessor: "gpu.3060Ti.Bitcoin Gold"
          },
          {
            Header: "Ethereum Classic",
            accessor: "gpu.3060Ti.Ethereum Classic"
          },
          {
            Header: "Flux",
            accessor: "gpu.3060Ti.Flux"
          },
          {
            Header: "Ergo",
            accessor: "gpu.3060Ti.Ergo"
          },
        ]
      },
    ],
