...
function App() {
  ...
  async function handlePriceChange(e) {
      ...
      await axios("/api/data/gpu/3060/"+p)
  ...
  async function resetPrice(e) {
      await axios("/api/data/gpu/3060/"+gpuPrice)
      const result = await axios("/api/data/rig")
      setData(result.data)
      setPrice(gpuPrice)
  }
  ...
  async function handleRigPriceChange(e) {
    ...
    await axios("/api/data/rig/3060/"+p)
    const result = await axios("/api/data/rig")
    console.log(result.data)
    setData(result.data)
    setServerP(p)
...
async function resetRigPrice(e) {
    await axios("/api/data/rig/3060/"+serverCost)
    const result = await axios("/api/data/rig")
    setData(result.data)
    ...
    () => [
      {
        ...
        columns: [
          ...
          {
            Header: "Ethereum",
            accessor: "gpu.3060.Ethereum"
          },
          {
            Header: "Conflux",
            accessor: "gpu.3060.Conflux"
          },
          {
            Header: "Ravencoin",
            accessor: "gpu.3060.Ravencoin"
          },
          {
            Header: "Bitcoin Gold",
            accessor: "gpu.3060.Bitcoin Gold"
          },
          {
            Header: "Ethereum Classic",
            accessor: "gpu.3060.Ethereum Classic"
          },
          {
            Header: "Flux",
            accessor: "gpu.3060.Flux"
          },
          {
            Header: "Ergo",
            accessor: "gpu.3060.Ergo"
          },
        ]
      },
    ],
